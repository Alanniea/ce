...
echo "✅ [2/6] 初始化 vnStat..."
# 对于 vnStat 2.x 版本，通常 vnstatd 守护进程启动时会自动创建数据库。
# 如果数据库不存在，第一次运行 'vnstat --update' 或 'vnstat' 也会尝试创建。
# 因此，移除 'vnstat -u -i "$IFACE" || true' 以避免旧版本参数错误。
sleep 2 # 给予vnstat一些时间来创建数据库文件，或者让服务启动

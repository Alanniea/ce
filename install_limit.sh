...
# 今日流量
VN=$(vnstat --oneline -i "$IFACE" 2>/dev/null)
TODAY_LINE=$(echo "$VN" | grep "$(date '+%Y-%m-%d')" | head -n1)
# 格式：Date;rx GiB;tx GiB;...
TX=$(echo "$TODAY_LINE" | cut -d\; -f3 | sed 's/ GiB//')
RX=$(echo "$TODAY_LINE" | cut -d\; -f4 | sed 's/ GiB//')
UP_STR="上行: ${TX:-0} GiB"
...
# 当前使用比
USED=$(vnstat --oneline -i "$IFACE" | cut -d\; -f11 | sed 's/ GiB//')
PCT=$(awk -v u="$USED" -v l="$LIMIT_GB" 'BEGIN{printf "%.1f", u/l*100}')
...
echo -e "${GREEN}📈 已用：${USED} GiB / ${LIMIT_GB} GiB (${PCT}%)${RESET}"

...
echo "0 0 * * * /root/clear_limit.sh && vnstat $VNSTAT_CREATE_OPT -i $IFACE && vnstat --update -i $IFACE" >> /tmp/crontab.bak
...
while true; do
  ...
  case "$opt" in
    ...
    5)
      ...
      crontab -l 2>/dev/null | grep -vE 'limit_bandwidth.sh|speed_test.sh' | crontab -

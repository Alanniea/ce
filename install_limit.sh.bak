...
echo "✅ [2/6] 初始化 vnStat..."  
vnstat --add -i "$IFACE" || true # 使用 --add 替代 -u  
sleep 2  
...
while true; do  
  ...
  case "$opt" in  
    ...
    4) vnstat -d ;;  
    5) rm -f /root/install_limit.sh /root/limit_bandwidth.sh /root/clear_limit.sh /usr/local/bin/ce  
...
echo -e "🎉 安装完成！现在可以使用命令：\033[1;32mce\033[0m 来管理流量限速。"